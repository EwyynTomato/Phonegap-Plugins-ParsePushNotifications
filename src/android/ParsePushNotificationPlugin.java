...
public class ParsePushNotificationPlugin extends CordovaPlugin
{
   ...
   public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException
   {
      ...
      else if (action.equalsIgnoreCase("subscribeToChannel"))
      {
         ...
         PushService.subscribeInBackground(channel);
      ...
      else if (action.equalsIgnoreCase("unsubscribeFromChannel"))
      {
         ...
         PushService.unsubscribeInBackground(channel);
   ...
   public static void NotificationReceived(String json, boolean receivedInForeground)
   {
      ...
      /*
       * 
       * THE following is the comment from the iOS version explaining the motivation for copying the 'alert' files into data.message in case there is no
       * explicit one set.
       * 
       ...
       * -------------------
       * 
       * on iOS we must have the alert field set on the wrapping aps hash. in addition as we have severe limitation on the size of the payload we would normally
       * avoid duplicating the notification text in both the aps wrapper and the payload object itself.
       * 
       * in order to keep the interface identical between platforms the aps.alert value is required in order for the ios notification center to have something
       * to show or else it wouls show the full JSON payload.
       * 
       * however on the js side we want to access all the properties for this notification inside a single object and care not for ios specific implemenataion
       * such as the aps wrapper
       * 
       * we could just duplicate the text and have it in both *aps.alert* and inside data.message but as the payload size limit is only 256 bytes it is better
       * to check if an explicit data.message value exists and if not just copy aps.alert into it
       */
